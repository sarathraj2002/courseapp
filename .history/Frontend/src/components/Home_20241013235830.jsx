...
const Home = () => {
  ...
  //  connecting external API
         useEffect(()=>{
          ...
          axios.delete('https://localhost:3000/course/coursedelete'+p).then((res)=>{
            alert ('deleted');
      ...
      {courses.map((course) => (
        <Grid item xs={12} sm={6} md={4} key={course.courseId}>
          <Card sx={{ width: 400, height: 700 ,marginLeft:2 }}>
            ...
            <CardActions >
          <Button size="large" color="primary" sx={{ backgroundColor: '#4caf50', color: 'white' }} onClick={() => handleEdit(course.courseId)}>
            Edit
          </Button>
          <Button size="large" color="secondary" varient="contained" sx={{ backgroundColor: 'red', color: 'white' }} onClick={() => {deleteCourse(course._id)}}>
            Delete
          </Button>
        </CardActions>
          </Card>
